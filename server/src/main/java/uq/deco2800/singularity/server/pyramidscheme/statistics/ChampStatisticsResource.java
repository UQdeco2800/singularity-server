...
@Path(ServerConstants.CHAMP_STATISTICS_RESOURCE)
@Produces(MediaType.APPLICATION_JSON)
public class ChampStatisticsResource {
	...
	private final ChampStatisticsDao statDao;
	private final UserDao userDao;
	private final TokenDao tokenDao;
	
	public ChampStatisticsResource(ChampStatisticsDao statDao, UserDao userDao, TokenDao tokenDao) {
		if(statDao == null) {
	 ...
	 */
	@GET
	public List<ChampionStatistics> getCurrentChampStatistics(
		...
		List<ChampionStatistics> champStats = null;
		
		if(userID.isPresent() && name.isPresent()) {
			champStats = this.statDao.findCurrentStatistics(userID.get(), name.get());
		}
		
		if(champStats != null) {
			return champStats;
		} else {
			throw new WebApplicationException("Champion statistics could not" +
	...
	public Response createNewChampStats(String payload, @QueryParam("token") Optional<String> tokenID) 
		...
		ChampionStatistics newChampStats = null;
		
		try {
			newChampStats = MAPPER.readValue(payload, ChampionStatistics.class);
		...
		try {
			userDao.findById(newChampStats.getUserID());
		} catch (Exception e) {
		...
		newChampStats.setStatID(statID);
		
		statDao.insert(newChampStats);
